...
def main(ngpus_per_node):
    ...
    # cache_client.Reset()
    # cache_client.ConstructNid2Pid(args.dataset, args.world_size // ngpus_per_node, 'metis', len(fg_train_mask))

    if args.distributed:
        # total # of DDP training process
        args.world_size = ngpus_per_node * args.world_size
        mp.spawn(run, nprocs=ngpus_per_node,
                 args=(ngpus_per_node, args, log_queue,fg))
    else:
        # run(0, ngpus_per_node, args)
        sys.exit(-1)


def run(gpu, ngpus_per_node, args, log_queue,fg):
    #################### 配置logger ####################
...
if __name__ == '__main__':
    ...
    if len(args.sampling.split('-')) > 10:
        ...
        log_filename = os.path.join(log_dir, f'multi_default_{model_name}_{datasetname}_trainer{args.world_size}_bs{args.batch_size}_sl{fanout}x{sampling_len}_ep{args.epoch}_hd{args.hidden_size}_localFalse.log')
    else:
        log_filename = os.path.join(log_dir, f'multi_default_{model_name}_{datasetname}_trainer{args.world_size}_bs{args.batch_size}_sl{args.sampling}_ep{args.epoch}_hd{args.hidden_size}_localFalse.log')
