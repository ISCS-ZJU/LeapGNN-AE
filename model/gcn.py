...
class NodeUpdate(nn.Module):
    ...
    def forward(self, node):
        ...
        # skip connection
        if self.concat:
            h = torch.cat((h, self.activation(h)), dim=1) # 把activation后的h和h连接起来
...
class GCNSampling(nn.Module):
    ...
    def forward(self, nf):
        ...
        for i, layer in enumerate(self.layers):
            ...
            if self.dropout:
                h = self.dropout(h)
            nf.layers[i].data['h'] = h # nf的第0层的h值进行赋值为features的值，这样可以在NodeUpdate中进行更新
            nf.block_compute(i, # nf的第0层进行计算
                             fn.copy_src(src='h', out='m'), # message function -> 直接传输不用做处理
                             lambda node : {'h': node.mailbox['m'].mean(dim=1)}, # reduce function -> 在dim1上求均值
