...
class P3_GraphSageSampling(nn.Module):
  def __init__(self,
               in_feats,
               n_hidden,
               n_classes,
               ...
               preprocess=False):
    super(P3_GraphSageSampling, self).__init__()
  ...
  def forward(self, nfs, rank):
    ...
    for lid, layer in enumerate(self.layers):
      mp_out = [[] for _ in range(len(nfs))]
      if lid == 0:
        for nf_id, nf in enumerate(nfs):
