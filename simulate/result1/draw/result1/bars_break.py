        ...
        for i in range(ncomprs):
            ...
            for j in range(len(legends[f'fig{fid}'][i])):
                ...
                if lname in legend_clr_map:
                    ...
                    axs[fid].bar(np.array(xcoor)+args.barwidth*i, grpdata[f'c{i}'][j], color=clrs[clr_idx], width=args.barwidth, bottom = btm_yvalue, edgecolor='black', linewidth=1.2)
                else:
                    clr_idx += 1
                    axs[fid].bar(np.array(xcoor)+args.barwidth*i, grpdata[f'c{i}'][j], color=clrs[clr_idx], width=args.barwidth, label=lname, bottom = btm_yvalue, edgecolor='black', linewidth=1.2)
