...
def get_sub_graph(dgl_g, train_nid, num_hops):
    ...
    for i in range(nf.num_blocks):
        nf_src_nids, nf_dst_nids, _ = nf.block_edges(
            i, remap_local=False)  # nodeflow-level id
        full_edge_src.append(nf.map_to_parent_nid(
            nf_src_nids))  # map to parent id == onid
    ...
    # 例如 [0 1 5 6 8] 表示当前Batch对应的sub graph中的0对应全局的0，2对应全局的5
    Print("lnid->onid", sub2full)
    ...
    coo_adj = spsp.coo_matrix((data, (sub_srcs, sub_dsts)), shape=(
        vnum, vnum))  # 使用子图的local id进行构建graph
    csr_adj = coo_adj.tocsr()  # remove redundant edges
    enum = csr_adj.data.shape[0]
    csr_adj.data = np.ones(enum, dtype=np.uint8)
    # 去重后的点和边的数量，即子图中实际上的点和边的数量
