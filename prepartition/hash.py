...
if __name__ == "__main__":
  ...
  partition_dataset = os.path.join(args.dataset, '{}naive'.format(args.partition)) # 存放分割后的graph
  try:
  ...
  for pid in range(args.partition):
    ...
    subadj, sub2fullid, subtrainid = get_sub_graph(dgl_g, part_nid, args.num_hops) # 根据子图分配到的点，进行邻居采样，扩充Part
