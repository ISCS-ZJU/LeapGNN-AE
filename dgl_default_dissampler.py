...
def run(gpu, ngpus_per_node, args):
    ...
    subg_topo_name = f'sub_coo_adj_{args.rank}.npz'
    subg_adj = data.get_struct(args.dataset, subg_topo_name)
    ...
    subfg = DGLGraph(subg_adj, readonly=True)
    ...
    with torch.autograd.profiler.profile(enabled=(args.gpu == 0), use_cuda=True) as prof:
        with torch.autograd.profiler.record_function('total epochs time'):
            for epoch in range(args.epoch):
                ...
                sampler = dgl.contrib.sampling.DistNeighborSampler(subfg, args.batch_size, expand_factor=int(sampling[0]), num_hops=len(sampling)+1, neighbor_type='in', shuffle=True, num_workers=args.num_worker, seed_nodes=train_lnid, prefetch=True, add_self_loop=True)
